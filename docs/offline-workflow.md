# Offline processing and evaluation

This guide covers the Python processor, rolling prototype, and earlier listening trials. For the project's purpose, Chrome extension, and current direction, see the [main README](../README.md). Run the commands below from the repository root.
...
For WAV input, the default output is `<input-stem>_<mode>.wav`. For MOV or MP4 input, explicitly supply `--output` with a matching video extension, as above: the CLI currently defaults to a `.wav` filename even though its video path performs a video remux. The processor rejects using the input path as the output. MOV and MP4 processing use a bundled FFmpeg binary: it extracts the **first audio track** to stereo PCM, processes it, then copies the original video stream and encodes the new audio as AAC. It copies subtitle streams when present. Other audio tracks are not included in the output.
...
The 2-second setting delays output by 2 seconds of audio while allowing the existing gain smoothing to see approaching loud events. In the recorded local trial, the 734.12-second *Tears of Steel* soundtrack processed in 1.66 seconds (443× audio duration), and the resulting WAV matched the offline Balanced v3 WAV byte-for-byte. That speed measures the file-driven audio computation, **not** sound-device capture, playback, or video synchronization. This prototype produces audio only and uses the deterministic level-based mode without streaming classifier inference. The separate [Chrome prototype](../chrome-extension/README.md) now handles live tab capture with a different processor; it does not use this two-second buffer, which would delay audio relative to the video.

## Tears of Steel test clip

The files and measurements below record earlier local listening trials. Media and generated outputs are ignored by Git and are not included in a fresh checkout.
...
Classification estimates which types of sound are present. It does **not** separate dialogue from overlapping music or effects; its gain adjustments still affect the mixed soundtrack. This classifier is optional for offline processing and is not integrated into the Chrome extension.
